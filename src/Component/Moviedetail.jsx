...
export default function Moviedetail() {
    ...
    const { original_title, overview, poster_path, vote_average, release_date, backdrop_path, budget, popularity, runtime, tagline } = movies;
  ...
  return (
    <section>
        ...
        {!isLoading && !error && (
          <div>
            <div class="bg-blue-50 min-h-screen flex items-center justify-center px-4" style={{
  ...
  <div class="max-w-[1200px] mx-auto flex flex-col md:flex-row bg-white shadow-lg rounded-lg overflow-hidden" >
    ...
    <div class="w-full md:w-2/3 p-6">
      ...
      <h1 class="text-2xl font-bold text-gray-800">
       {original_title}<span class="text-gray-500"> ({release_date ? release_date.split("-")[0] : "N/A"})</span>
      </h1>
      <p class="mt-2 text-sm text-gray-600">Release Date: {release_date}</p>
      ...
      <div class="flex items-start mt-4 flex-col md:flex md:flex-row gap-4 ">
        <div class="flex items-center space-x-2">
          <div class="w-10 h-10 rounded-full bg-green-500 text-white flex items-center justify-center font-bold">
            <h1>{popularity ? `${Math.round(popularity)}%` : "N/A"}</h1>
          </div>
          <span class="text-sm text-gray-400"><span className='text-[20px] text-gray-500'>Budget: </span> {formatBudget(budget)}</span>
        </div>
  ...
  <p className='pt-4 font-medium'>Tagline: <span className='font-normal'>{tagline}</span></p>
      {/* <!-- Overview --> */}
      <div class="mt-6">
        ...
        <p class="mt-4 text-sm text-gray-500">
          <span class="font-bold">Runtime : </span>{formatRuntime(runtime)}
        </p>
      </div>
    </div>
  </div>
